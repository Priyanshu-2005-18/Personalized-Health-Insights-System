...
services/scorer.py
==================
Scoring logic — converts raw health metrics into 0–100 sub-scores
and derives a composite health score when one is not supplied.

Sub-score formula (per metric):
  Each metric is mapped to a 0–100 scale based on evidence-based
  optimal ranges. Values outside the optimal range receive a graduated
  penalty rather than a hard cutoff.

Composite score (when health_score is None):
  Weighted average of all available sub-scores.
  Weights match the importance assigned in thresholds.CATEGORY_WEIGHTS.
"""

from typing import Dict, Optional
from app.models.health import HealthMetrics, HealthScoreLabel, MetricStatus
...
# ─────────────────────────────────────────────────────────────────────────────
#  Individual metric sub-scores (0–100)
# ─────────────────────────────────────────────────────────────────────────────
...
def score_heart_rate(bpm: int) -> float:
    ...
    if bpm <= HR.athlete_max:
        return 95.0   # athlete-level — near perfect, slight monitor flag
...
def get_health_score_label(score: float) -> HealthScoreLabel:
    # Bands: 0–40 Critical, 40–55 Poor, 55–70 Fair, 70–85 Good, 85–100 Excellent
    if score >= SCORE_BANDS.excellent_min:   # >= 85
        return HealthScoreLabel.EXCELLENT
    if score >= SCORE_BANDS.fair_max:        # >= 70
        return HealthScoreLabel.GOOD
    if score >= SCORE_BANDS.poor_max:        # >= 55
        return HealthScoreLabel.FAIR
    if score >= SCORE_BANDS.critical_max:    # >= 40
